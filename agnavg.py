...
zrange = np.arange(0.01, 0.1, 0.002)
# wavelengths to interpolate spectra
lambdas = np.arange(811.0, 2420.0, 0.5) 

agns = at.read('spectra/AGN/properties-edit.txt')
fileroot = 'spectra/AGN/'
plotroot = 'figs/agns/'

specarr = np.zeros((agns.size, lambdas.size),np.float)

for n, gal in enumerate(agns.Name):
    #if n == 1:
    #    break
    #print 'Reading AGN '+gal
    agnraw = np.loadtxt(fileroot+gal+'.txt')
    # convert from ergs s-1 cm-2 A-1 to ergs s-1 m-2 nm-1
    agnflux = agnraw[:,1]*1.0e5
    # Scale the agn flux down to J=15 mag
    agnscfl = agnflux * 10**(-(15.0-agns.J[n])/2.5)
    # table has wavelength in microns, convert to nm & 
    # shift lambda to rest frame
    agnlamb = agnraw[:,0]*1000.0/(1+agns.z[n])   
...
for i, z in enumerate(zrange):
    ...
    for j, l in enumerate(lams):
        ...
        for k, m in enumerate(mags):
            ...
            exptarr[i,j,k] = ((sn**2*(Ragn + Rsky) + 
                               np.sqrt(sn**4*(Ragn + Rsky)**2 +
                                       4*sn**2*Ragn**2*Rn2n))
                              /(2*Ragn**2))

            snrarr[i,j,k] = ((Ragn*ft)/
...
ax.set_xticklabels([])
ax.grid(True)
ax.plot(lambdas, medflux)
ax.set_ylabel(r'Flux - ergs s$^{-1}$ m$^{-2}$ nm$^{-1}$')
ax.set_yticks([2e-11,4e-11,6e-11])
ax.text(0.5, 0.9,'Spectrum and SNR for Type I AGN', ha='center', 
...
lo_ticks = np.array([900., 1100., 1300., 1500., 1700.])
new_tick_locations = (lo_ticks-xmin)/(xmax-xmin)
...
ax1.plot([J['start'],J['start']],[ymin,ymax],'b--',linewidth=1)
ax1.plot([J['end'],J['end']],[ymin,ymax],'b--',linewidth=1)
ax1.text(J['cwvl']*1e9, 0.1, 'H', color='b')

ax1.plot([H['start'],H['start']],[ymin,ymax],'g--',linewidth=1)
ax1.plot([H['end'],H['end']],[ymin,ymax],'g--',linewidth=1)
ax1.text(H['cwvl']*1e9, 0.1, 'H', color='g')
...
ax3.plot([H['start'],H['start']],[ymin,ymax],'g--',linewidth=1)
ax3.plot([H['end'],H['end']],[ymin,ymax],'g--',linewidth=1)
ax3.text(H['cwvl']*1e9, 0.1, 'H', color='g')
