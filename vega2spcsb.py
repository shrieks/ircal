...
vega = np.loadtxt('vega/alpha_lyr_stis_005.txt')
...
# wavelength spacing. Need lambda/2R only for Nyquist reconstruction
space = 10 # nm
...
pmj,= mp.plot(mlamJ, vegamos['limmag'][0:lmj], 'r-')
ph, = mp.plot(lamH, vegaspec['limmag'][lj:lj+lh], 'b--')
pmh,= mp.plot(mlamH, vegamos['limmag'][lmj:lmj+lmh], 'b-')
pks,= mp.plot(lamKs, vegaspec['limmag'][lj+lh:lj+lh+lks], 'g--')
pmks,=mp.plot(mlamKs, vegamos['limmag'][lmj+lmh:lmj+lmh+lmks], 'g-')
mp.xlabel('Wavelength (nm)')
mp.ylabel('Magnitudes')
mp.title('Spectroscopy magnitude limits, R=500')
leg1 = mp.legend([pmj,pmh,pmks],['J','H','Ks'], loc='lower right', title='Sh+MOs')
mp.gca().add_artist(leg1)
mp.legend([pj, ph, pks],['J', 'H','Ks'], loc='lower left', title='ShARCS')
...
mp.legend(curves, lam2, loc='upper left', ncol=2, title='IRCAL Filters')
...
for lam in mlam2:
    ...
    curves.append(p)
mp.legend(curves, mlam2, loc='upper left', ncol=2, title='MOSFIRE Filters')
mp.plot([min(mags),max(mags)],[300,300],'-', linewidth=2)
mp.text(np.min(mags)+0.1, 350, '300s Fowler')
mp.text(np.min(mags)+0.1, 175, 'Exposure')
